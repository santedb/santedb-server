...
UPDATE ENT_VRSN_TBL SET CLS_CD_ID = ENT_TBL.CLS_CD_ID, TPL_ID = ENT_TBL.TPL_ID, DTR_CD_ID = ENT_TBL.DTR_CD_ID
...
BEGIN
	RETURN (SELECT CASE WHEN COUNT(*) > 0 THEN TRUE ELSE FALSE END AS r FROM ENT_VRSN_TBL INNER JOIN CD_CUR_VRSN_VW ON (ENT_VRSN_TBL.CLS_CD_ID = CD_CUR_VRSN_VW.CD_ID) WHERE ENT_ID = :ENT_ID_IN AND CD_CUR_VRSN_VW.MNEMONIC = :CLS_MNEMONIC_IN);
END;--#!
