	...
	public class ConfigurationSectionHandler : IConfigurationSectionHandler
	{
		...
		public object Create(object parent, object configContext, XmlNode section)
		{
			...
			if (smtp == null)
				...
				retVal.Smtp = new SmtpConfiguration(new Uri(smtp.Attributes["server"]?.Value ?? "smtp://localhost:25"), smtp.Attributes["username"]?.Value ?? string.Empty, smtp.Attributes["password"]?.Value ?? string.Empty, Boolean.Parse(smtp.Attributes["ssl"]?.Value ?? "false"), smtp.Attributes["from"]?.Value ?? "no-reply@SanteDB.org");
