...
namespace SanteDB.Messaging.HDSI.ResourceHandler
{
	...
	/// </summary>
	public class ReferenceTermResourceHandler : IResourceHandler
	{
		/// <summary>
		...
		/// </summary>
		public Type Type => typeof(ReferenceTerm);
        ...
        public Object Create(Object data, bool updateIfExists)
		{
			...
			{
				throw new InvalidOperationException($"Bundle must have entry of type {nameof(ReferenceTerm)}");
			}
			else if (processData is ReferenceTerm)
			{
				var referenceTerm = data as ReferenceTerm;

				if (updateIfExists)
				{
					return this.repository.SaveReferenceTerm(referenceTerm);
				}
				else
				{
					return this.repository.InsertReferenceTerm(referenceTerm);
				}
			}
			else
			{
        ...
        public Object Update(Object  data)
		{
			...
			if (processData is Bundle)
			{
				throw new InvalidOperationException(string.Format("Bundle must have entry of type {0}", nameof(ReferenceTerm)));
			}
			else if (processData is ReferenceTerm)
			{
				var referenceTermData = data as ReferenceTerm;

				return this.repository.SaveReferenceTerm(referenceTermData);
			}
			else
			{
				throw new ArgumentException("Invalid persistence type");
