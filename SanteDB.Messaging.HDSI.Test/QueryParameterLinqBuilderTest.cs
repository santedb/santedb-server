...
namespace SanteDB.Messaging.HDSI.Test
{
    ...
    public class QueryParameterLinqBuilderTest
    {
        ...
        /// <summary>
        /// Test that building a sub-expression guard works
        /// </summary>
        [Test]
        public void TestGuardSubExpression()
        {
            var filterQuery = "relationship[relationshipType.conceptSet.mnemonic%3dFamilyMember].target@Person.identifier.value=3049";
            var expr = QueryExpressionParser.BuildLinqExpression<Patient>(filterQuery);
            var expected = "o => o.Relationships.Where(guard => guard.RelationshipType.ConceptSets.Any(conceptSet => (conceptSet.Mnemonic == \"FamilyMember\"))).Any(relationship => ((relationship.TargetEntity As Person) ?? new Person()).Identifiers.Any(identifier => (identifier.Value == \"3049\")))";
        ...
        {
            string expected = "o => ((o.DateOfBirth != null) AndAlso (o.DateOfBirth.Value.TestExpressionEx((Invoke(__xinstance => (((__xinstance.LoadCollection(\"Relationships\", False, Convert(null)).Where(guard => ((guard.LoadProperty(\"RelationshipType\", False, Convert(null)) ?? new Concept()).Mnemonic == \"Mother\")).FirstOrDefault() ?? new EntityRelationship()).LoadProperty(\"TargetEntity\", False, Convert(null)) As Patient) ?? new Patient()).DateOfBirth, o) ?? default(DateTime))) > 7300.00:00:00))";
            if (Environment.OSVersion.Platform != PlatformID.Win32NT)
            {
                expected = "o => ((o.DateOfBirth != null) AndAlso (o.DateOfBirth.Value.TestExpressionEx((Invoke(__xinstance => (((__xinstance.LoadCollection(\"Relationships\", False, Convert(null, IEnumerable`1)).Where(guard => ((guard.LoadProperty(\"RelationshipType\", False, Convert(null, IEnumerable`1)) ?? new Concept()).Mnemonic == \"Mother\")).FirstOrDefault() ?? new EntityRelationship()).LoadProperty(\"TargetEntity\", False, Convert(null, IEnumerable`1))) As Patient) ?? new Patient()).DateOfBirth, o) ?? default(DateTime))) > 7300.00:00:00))";
            }
