...
namespace SanteDB.Persistence.Data.ADO.Services
{
    ...
    public class AdoPersistenceService : IDaemonService, ISqlDataPersistenceService, IBisDataSource
    {
        ...
        public BisResultContext ExecuteQuery(BisQueryDefinition queryDefinition, Dictionary<string, object> parameters)
        {
            ...
            var provider = Activator.CreateInstance(queryDefinition.DataSources.First().ProviderType) as IDbProvider;
            ...
            using (var context = provider.GetReadonlyConnection())
            {
                try
                {
                    ...
                    var results = context.Query<ExpandoObject>(new SqlStatement(provider, stmt, values));
