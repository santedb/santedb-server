...
CREATE TABLE SEC_DEV_TBL (
	...
	CRT_PROV_ID UUID NOT NULL, -- THE USER WHICH CREATED TEH DEVICE
	UPD_UTC TIMESTAMP, -- THE DATE THE DEVICE WAS CREATED
	UPD_PROV_ID UUID, -- THE USER WHICH CREATED TEH DEVICE
...
CREATE TABLE PHON_VAL_TBL (
	VAL_ID INTEGER NOT NULL, -- UNIQUE IDENTIFIER FORHTE VALUE
...
CREATE TRIGGER TG_PHON_VAL_TBL_SEQ FOR PHON_VAL_TBL ACTIVE BEFORE INSERT POSITION 0 AS BEGIN
	NEW.VAL_ID = NEXT VALUE FOR PHON_VAL_SEQ;
END!!
SET TERM ; !!
...
SET TERM !!;
CREATE TRIGGER TG_CD_TBL_SEQ FOR CD_TBL ACTIVE BEFORE INSERT POSITION 0 AS BEGIN
	NEW.CD_ID = NEXT VALUE FOR CD_ID_SEQ;
END!!
SET TERM ; !!
...
CREATE UNIQUE INDEX CD_SET_UUID_IDX ON CD_SET_TBL(UUID);
CREATE UNIQUE INDEX CD_UUID_IDX ON CD_TBL(UUID);

SET TERM !!;
CREATE TRIGGER TG_CD_SET_TBL_SEQ FOR CD_SET_TBL ACTIVE BEFORE INSERT POSITION 0 AS BEGIN
	NEW.SET_ID = NEXT VALUE FOR CD_SET_ID_SEQ;
END!!
SET TERM ; !!
...
CREATE TABLE CD_SET_MEM_ASSOC_TBL (
	SET_ID INTEGER NOT NULL, -- THE CONCEPT SET TO WHICH THE CONCEPT BELONGS
	CD_ID INTEGER NOT NULL, -- THE CONCEPT WHICH BELONGS TO THE SET.,
...
CREATE TABLE CD_VRSN_TBL (
	VRSN_UUID UUID NOT NULL, 
	VRSN_SEQ_ID INTEGER UNIQUE NOT NULL,-- VERSION SEQUENCE
	CD_ID INTEGER NOT NULL, -- THE CONCEPT TO WHICH THE VERSION APPLIES
	STS_CD_ID INTEGER NOT NULL, -- THE STATUS OF THE CONCEPT AT THE CURRENT STATE
	...
	CONSTRAINT PK_CD_VRSN_TBL PRIMARY KEY (VRSN_UUID),
	...
	CONSTRAINT FK_CD_VRSN_RPLC_VRSN_ID FOREIGN KEY (RPLC_VRSN_ID) REFERENCES CD_VRSN_TBL(VRSN_UUID),
...
CREATE TABLE CD_NAME_TBL (
	...
	CD_ID INTEGER NOT NULL, -- THE CONCEPT THAT THE NAME APPLIES TO
...
CREATE SEQUENCE REF_TERM_ID_SEQ;

-- REFERENCE TERMS
CREATE TABLE REF_TERM_TBL (
	UUID UUID NOT NULL,
	REF_TERM_ID INTEGER NOT NULL,
...
SET TERM !!;
CREATE TRIGGER TG_REF_TERM_TBL_SEQ FOR REF_TERM_TBL ACTIVE BEFORE INSERT POSITION 0 AS BEGIN
	NEW.REF_TERM_ID = NEXT VALUE FOR REF_TERM_ID_SEQ;
END!!
SET TERM ; !!
...
CREATE TABLE CD_REF_TERM_ASSOC_TBL (
	...
	REF_TERM_ID INTEGER NOT NULL, -- THE REFERENCE TERM PART OF THIS RELATIONSHIP
	CD_ID INTEGER NOT NULL, -- THE CONCEPT DEFINITION IDENTIFIER
...
CREATE TABLE REF_TERM_NAME_TBL (
	...
	REF_TERM_ID INTEGER NOT NULL, -- THE REFERENCE TERM ID TO WHICH THE NAME APPLIES
...
CREATE TABLE CD_REL_ASSOC_TBL (
	...
	SRC_CD_ID INTEGER NOT NULL, -- THE SOURCE OF THE RELATIONSHIP
	TRG_CD_ID INTEGER NOT NULL, -- THE TARGET OF THE RELATIONSHIP
...
CREATE VIEW CD_SET_MEM_VW AS
	SELECT CD_SET_TBL.SET_ID, CD_SET_TBL.MNEMONIC AS SET_MNEMONIC, CD_CUR_VRSN_VW.CD_ID, CD_CUR_VRSN_VW.VRSN_UUID, CD_CUR_VRSN_VW.MNEMONIC AS CD_MNEMONIC, CD_CUR_VRSN_VW.CLS_MNEMONIC
...
BEGIN
	RETURN (SELECT CASE WHEN COUNT(*) > 0 THEN TRUE ELSE FALSE END AS r FROM CD_SET_MEM_ASSOC_TBL INNER JOIN CD_SET_TBL USING(SET_ID) WHERE CD_ID = :CD_ID_IN AND (MNEMONIC = :SET_MNEMONIC_IN OR MNEMONIC = 'NullReason'));
END!!

CREATE FUNCTION CK_IS_CD_SET_MEM ( CD_ID_IN INTEGER, SET_MNEMONIC_IN VARCHAR(32), ALLOW_NULL_IN BOOLEAN) RETURNS BOOLEAN AS
 BEGIN 
...
CREATE TABLE ASGN_AUT_SCP_TBL (
	...
	CD_ID INTEGER NOT NULL, -- THE CHAR(16) FOR THE CLASS WHICH DESCRIBES THE SCOPE
...
CREATE TABLE ID_TYP_TBL (
	...
	TYP_CD_ID INTEGER NOT NULL, -- THE TYPE OF IDENTIFIER
	ENT_SCP_CD_ID INTEGER NOT NULL, -- THE SCOPE IN WHICH THE IDENTIFIER TYPE CAN BE USED
	CRT_UTC TIMESTAMP DEFAULT CURRENT_TIMESTAMP NOT NULL, -- THE TIME THAT THE AA WAS CREATED
	CRT_PROV_ID UUID NOT NULL, -- THE USER WHICH CREATED THE AA
	OBSLT_UTC TIMESTAMP, -- THE TIME THAT THE AA WAS OBSOLETED
	OBSLT_PROV_ID UUID, -- THE USER WHICH OBSOLETED THE AA
...
CREATE TABLE ACT_TBL (
	...
	CLS_CD_ID INTEGER NOT NULL, -- CLASSIFIES THE TYPE OF ACT
	MOD_CD_ID INTEGER NOT NULL, -- IDENTIIFES THE MOOD OF THE ACT
...
SET TERM !!;
CREATE TRIGGER TG_ACT_TBL_SEQ FOR ACT_TBL ACTIVE BEFORE INSERT POSITION 0 AS BEGIN
	NEW.ACT_ID = NEXT VALUE FOR ACT_ID_SEQ;
END!!
SET TERM ; !!
...
CREATE TABLE ACT_VRSN_TBL (
	...
	ACT_ID INTEGER NOT NULL, -- THE ACT IDENTIFIER TO WHICH THIS VERSION APPLIES
	...
	TYP_CD_ID INTEGER, -- THE TYPE CONCEPT OF THE ACT AT THIS VERSION
	STS_CD_ID INTEGER NOT NULL, -- THE STATUS OF THE ACT AS OF THIS VERSION
	RSN_CD_ID INTEGER, -- THE REASON OF THE ACT TO OCCUR
	...
	CONSTRAINT PK_ACT_VRSN_TBL PRIMARY KEY (VRSN_UUID),
	...
	CONSTRAINT FK_ACT_VRSN_RPLC_VRSN_ID FOREIGN KEY (RPLC_VRSN_ID) REFERENCES ACT_VRSN_TBL(VRSN_UUID),
...
CREATE TABLE ACT_ID_TBL (
	...
	ACT_ID INTEGER NOT NULL, -- THE IDENTIFIER OF THE ACT
...
CREATE TABLE ACT_REL_TBL (
	...
	SRC_ACT_ID INTEGER NOT NULL, -- THE SOURCE ACT
	TRG_ACT_ID INTEGER NOT NULL, -- THE TARGET ACT
	...
	REL_TYP_CD_ID INTEGER NOT NULL, -- THE RELATIONSHIP TYPE CONCEPT
...
CREATE TABLE ACT_EXT_TBL (
	...
	ACT_ID INTEGER NOT NULL, -- THE ACT TO WHICH THE EXTENSION APPLIES
...
CREATE TABLE CNTRL_ACT_TBL (
	VRSN_SEQ_ID INTEGER NOT NULL, -- THE LINK TO THE ACT WHICH IS A CONTROL ACT
	CONSTRAINT PK_CNTRL_ACT_TBL PRIMARY KEY (VRSN_SEQ_ID),
	CONSTRAINT FK_CNTRL_ACT_ACT_VRSN_ID FOREIGN KEY (VRSN_SEQ_ID) REFERENCES ACT_VRSN_TBL(VRSN_SEQ_ID)
);

-- PROCEDURES TABLE
CREATE TABLE PROC_TBL (
	VRSN_SEQ_ID INTEGER NOT NULL, -- THE UUID FOR THE ACT VERSION INFORMATION
	MTH_CD_ID INTEGER, -- THE UUID FOR THE METHOD / TECHNIQUE CODE
	APR_STE_CD_ID INTEGER, -- THE UUID FOR THE APPOACH SITE CODE
	TRG_STE_CD_ID INTEGER, -- THE UUID FOR THE TARGET SITE CODE
	CONSTRAINT PK_PROC_TBL PRIMARY KEY (VRSN_SEQ_ID),
	CONSTRAINT FK_PROC_VRSN_ID FOREIGN KEY (VRSN_SEQ_ID) REFERENCES ACT_VRSN_TBL(VRSN_SEQ_ID),
...
CREATE TABLE OBS_TBL (
	VRSN_SEQ_ID INTEGER NOT NULL, -- THE VERSION TO WHICH THIS OBSERVATION DATA APPLIES
	INT_CD_ID INTEGER, -- THE INTERPRETATION CODE OF THE OBSERVATION
	VAL_TYP VARCHAR(2) CHECK (VAL_TYP IN ('PQ','ST','CD')) NOT NULL, -- THE TYPE OF VALUE
	CONSTRAINT PK_OBS_TBL PRIMARY KEY (VRSN_SEQ_ID),
	CONSTRAINT FK_OBS_ACT_VRSN_ID FOREIGN KEY (VRSN_SEQ_ID) REFERENCES ACT_VRSN_TBL(VRSN_SEQ_ID),
...
CREATE TABLE QTY_OBS_TBL (
	VRSN_SEQ_ID INTEGER NOT NULL, -- THE VERSION TO WHICH THE OBSERVATION DATA APLIES
	QTY DECIMAL NOT NULL, -- THE QUANTITY ITSELF
	QTY_PRC NUMERIC(2), -- THE PRECISION OF THE OBSERVED VALUES
	UOM_CD_ID INTEGER NOT NULL, -- THE UNIT OF MEASURE OF THE QUANTITY
	CONSTRAINT PK_QTY_OBS_TBL PRIMARY KEY (VRSN_SEQ_ID),
	CONSTRAINT FK_QTY_OBS_ACT_VRSN_ID FOREIGN KEY (VRSN_SEQ_ID) REFERENCES OBS_TBL(VRSN_SEQ_ID),
...
CREATE TABLE CD_OBS_TBL (
	VRSN_SEQ_ID INTEGER NOT NULL, -- THE VERSION TO WHICH THE OBSERVATION DTA APPLIES
	VAL_CD_ID INTEGER NOT NULL, -- THE CONCEPT WHICH REPRESENTS THE VALUE
	CONSTRAINT PK_CD_OBS_TBL PRIMARY KEY (VRSN_SEQ_ID),
	CONSTRAINT FK_CD_OBS_ACT_VRSN_ID FOREIGN KEY (VRSN_SEQ_ID) REFERENCES OBS_TBL(VRSN_SEQ_ID),
...
CREATE TABLE TXT_OBS_TBL (
	VRSN_SEQ_ID INTEGER NOT NULL, -- THE VERSION TO WHICH THE BLOB SUB_TYPE TEXT DATA APPLIES
	OBS_VAL BLOB SUB_TYPE TEXT NOT NULL, -- THE OBSERVATION VALUE 
	CONSTRAINT PK_TXT_OBS_TBL PRIMARY KEY (VRSN_SEQ_ID),
	CONSTRAINT FK_TXT_OBS_ACT_VRSN_ID FOREIGN KEY (VRSN_SEQ_ID) REFERENCES OBS_TBL(VRSN_SEQ_ID)
);

-- PATIENT ENCOUNTER
CREATE TABLE PAT_ENC_TBL (
	VRSN_SEQ_ID INTEGER NOT NULL, -- THE VERSION TO WHICH THE ENCOUNTER DATA APPLIES
	DSCH_DSP_CD_ID INTEGER, -- THE DISCHARGE DISPOSITION IF KNOWN
	CONSTRAINT PK_PAT_ENC_TBL PRIMARY KEY (VRSN_SEQ_ID),
	CONSTRAINT FK_PAT_ENC_ACT_VRSN_ID FOREIGN KEY (VRSN_SEQ_ID) REFERENCES ACT_VRSN_TBL(VRSN_SEQ_ID),
...
CREATE TABLE SUB_ADM_TBL (
	VRSN_SEQ_ID INTEGER NOT NULL, -- THE VERSION TO WHICH THE SUBSTANCE ADMINISTRATION DATA APPLIES
	STE_CD_ID INTEGER, -- THE SITE OF ADMINISTRATION
	RTE_CD_ID INTEGER NOT NULL, -- THE ROUTE OF ADMINISTRATION
	DOS_QTY DECIMAL NOT NULL, -- THE AMOUNT ADMINISTERED
	DOS_UNT_CD_ID INTEGER NOT NULL, -- THE DOSE UOM CODE
	SEQ_ID INT, -- THE SEQUENCE CODE OF THE DOSE IF GIVEN AS PART OF A DOSE
	CONSTRAINT PK_SUB_ADM_TBL PRIMARY KEY (VRSN_SEQ_ID),
	CONSTRAINT FK_SUB_ADM_ACT_VRSN_ID FOREIGN KEY (VRSN_SEQ_ID) REFERENCES ACT_VRSN_TBL(VRSN_SEQ_ID),
...
CREATE TABLE ACT_TAG_TBL (
	...
	ACT_ID INTEGER NOT NULL, -- THE ACT TO WHICH THE TAG APPLIES
...
CREATE TABLE ACT_POL_ASSOC_TBL (
	...
	ACT_ID INTEGER NOT NULL, -- THE ACT TO WHICH THE POLICY APPLIES
...
CREATE SEQUENCE ENT_ID_SEQ;
-- ENTITY TABLE
CREATE TABLE ENT_TBL (
	ENT_ID INTEGER NOT NULL,
	UUID UUID NOT NULL,
	TPL_ID UUID, -- THE TEMPLATE WHICH THIS ENTITY FOLLOWS
	CLS_CD_ID INTEGER NOT NULL, -- IDENTIIFES THE CLASS CONCEPT
	DTR_CD_ID INTEGER NOT NULL, -- IDENTIFIES THE ENTITY DETERMINER
...
SET TERM !!;
CREATE TRIGGER TG_ENT_TBL_SEQ FOR ENT_TBL ACTIVE BEFORE INSERT POSITION 0 AS BEGIN
	NEW.ENT_ID = NEXT VALUE FOR ENT_ID_SEQ;
END!!
SET TERM ; !!
...
CREATE TABLE ENT_TAG_TBL (
	TAG_ID UUID NOT NULL,
	ENT_ID INTEGER NOT NULL, -- THE ENTITY TO WHICH THE TAG APPLIES
	TAG_NAME VARCHAR(64) NOT NULL, -- THE NAME OF THE TAG
	TAG_VALUE BLOB SUB_TYPE TEXT, -- THE VALUE OF THE TAG
...
CREATE TABLE ENT_VRSN_TBL (
	...
	ENT_ID INTEGER NOT NULL, -- THE ENTITY TO WHICH THE VERSION APPLIES
	RPLC_VRSN_ID UUID, -- THE VERSION THAT THIS ENTITY VERSION REPLACES
	STS_CD_ID INTEGER NOT NULL, -- THE STATUS OF THE ENTITY AT THIS VERSION
	TYP_CD_ID INTEGER, -- THE SUBCLASS OR TYPE WITHIN THE CLASS OF THE ENTITY
	...
	CONSTRAINT PK_ENT_VRSN_TBL PRIMARY KEY (VRSN_UUID),
	CONSTRAINT FK_ENT_VRSN_ENT_ID FOREIGN KEY (ENT_ID) REFERENCES ENT_TBL(ENT_ID),
	CONSTRAINT FK_ENT_VRSN_RPLC_VRSN_SEQ_ID FOREIGN KEY (RPLC_VRSN_ID) REFERENCES ENT_VRSN_TBL(VRSN_UUID),
...
CREATE TABLE ENT_POL_ASSOC_TBL (
	SEC_POL_INST_ID UUID NOT NULL,
	ENT_ID INTEGER NOT NULL, -- THE ENT TO WHICH THE POLICY APPLIES
...
CREATE TABLE ENT_REL_TBL (
	...
	SRC_ENT_ID INTEGER NOT NULL, -- THE SOURCE ENTITY OF THE RELATIONSHIP
	TRG_ENT_ID INTEGER NOT NULL, -- THE TARGET ENTITY OF THE RELATIONSHIP
	...
	REL_TYP_CD_ID INTEGER NOT NULL, -- THE TYPE OF RELATIONSHIP BETWEEN THE ENTITIES
...
CREATE SEQUENCE ENT_ADDR_ID_SEQ;

-- ENTITY ADDRESS TABLE
CREATE TABLE ENT_ADDR_TBL (
	ADDR_ID INTEGER NOT NULL,
	ADDR_UUID UUID UNIQUE NOT NULL,
	ENT_ID INTEGER NOT NULL, -- THE ENTITY TO WHICH THE ADDRESS IS ATTACHED
	...
	USE_CD_ID INTEGER NOT NULL, -- INDICATES THE USE OF THE ADDRESS
...
SET TERM !!;
CREATE TRIGGER TG_ENT_ADDR_TBL_SEQ FOR ENT_ADDR_TBL ACTIVE BEFORE INSERT POSITION 0 AS BEGIN
	NEW.ADDR_ID = NEXT VALUE FOR ENT_ADDR_ID_SEQ;
END!!
SET TERM ; !!
...
CREATE TRIGGER TG_ENT_ADDR_CMP_VAL_TBL_SEQ FOR ENT_ADDR_CMP_VAL_TBL ACTIVE BEFORE INSERT POSITION 0 AS BEGIN
	NEW.VAL_ID = NEXT VALUE FOR ENT_ADDR_CMP_VAL_SEQ;
END!!
SET TERM ; !!

CREATE INDEX EN_ADDR_CMP_VAL_VAL_IDX ON ENT_ADDR_CMP_VAL_TBL(VAL);

CREATE SEQUENCE ENT_ADDR_CMP_ID_SEQ;

-- ENTITY ADDRESS COMPONENT TABLE
CREATE TABLE ENT_ADDR_CMP_TBL (
	CMP_ID INTEGER NOT NULL,
	TYP_CD_ID INTEGER, -- THE TYPE OF THE COMPONENT (STREET, CITY, ETC.)
	VAL_ID INTEGER NOT NULL, -- THE CHAR(16) FOR THE ADDRESS VALUE
	ADDR_ID INTEGER NOT NULL, -- THE CHAR(16) FOR THE ADDRESS TO WHICH THE COMPONENT APPLIES
...
CREATE TRIGGER TG_ENT_ADDR_CMP_TBL_SEQ FOR ENT_ADDR_CMP_TBL ACTIVE BEFORE INSERT POSITION 0 AS BEGIN
	NEW.CMP_ID = NEXT VALUE FOR ENT_ADDR_CMP_ID_SEQ;
END!!
SET TERM ; !!
...
CREATE SEQUENCE ENT_NAME_NAME_ID_SEQ;
-- ENTITY NAME TABLE
CREATE TABLE ENT_NAME_TBL (
	NAME_ID INTEGER NOT NULL,
	NAME_UUID UUID UNIQUE NOT NULL,
	ENT_ID INTEGER NOT NULL, -- THE ENTITY TO WHICH THE NAME APPLIES
	...
	USE_CD_ID INTEGER NOT NULL, -- THE USE OF THE NAME (LEGAL, ETC.)
...
SET TERM !!;
CREATE TRIGGER TG_ENT_NAME_TBL_SEQ FOR ENT_NAME_TBL ACTIVE BEFORE INSERT POSITION 0 AS BEGIN
	NEW.NAME_ID = NEXT VALUE FOR ENT_NAME_NAME_ID_SEQ;
END!!
SET TERM ; !!

CREATE SEQUENCE NAME_CMP_ID_SEQ;

-- ENTITY NAME COMPONENT
CREATE TABLE ENT_NAME_CMP_TBL (
	CMP_ID INTEGER NOT NULL,
	TYP_CD_ID INTEGER, -- THE TYPE OF COMPONENT (FAMILY, GIVEN, ETC);
	VAL_ID INTEGER NOT NULL, -- THE PHONETIC CODE FOR THE NAME
	NAME_ID INTEGER NOT NULL, -- THE NAME TO WHICH THE COMPONENT APPLIES
...
CREATE TRIGGER TG_ENT_NAME_CMP_TBL_SEQ FOR ENT_NAME_CMP_TBL ACTIVE BEFORE INSERT POSITION 0 AS BEGIN
	NEW.CMP_ID = NEXT VALUE FOR NAME_CMP_ID_SEQ;
END!!
SET TERM ; !!
...
CREATE TABLE ENT_EXT_TBL (
	...
	ENT_ID INTEGER NOT NULL, -- THE ENTITY TO WHICH THE EXTENSION APPLIES
...
CREATE TABLE ENT_ID_TBL (
	...
	ENT_ID INTEGER NOT NULL, -- THE ENTITY TO WHICH THE IDENTIFIER IS ATTACHED
...
CREATE SEQUENCE ENT_TEL_ID_SEQ;

-- TELECOM ADDRESS
CREATE TABLE ENT_TEL_TBL (
	TEL_ID UUID NOT NULL,
	ENT_ID INTEGER NOT NULL, -- THE ENTITY TO WHICH THE TELECOM ADDRESS APPLIES
	TYP_CD_ID INTEGER, -- THE TYPE OF ADDRESS (FAX, TEL, EMAIL, PAGER, ETC.)
	USE_CD_ID INTEGER NOT NULL, -- IDENTIIFES THE USE OF THE ADDRESS (HOME, WORK, ETC.)
...
CREATE TABLE PLC_TBL (
	VRSN_SEQ_ID INTEGER NOT NULL, -- THE ENTITY VERSION THIS PLACE REPRESENTS
	...
	CONSTRAINT PK_PLC_TBL PRIMARY KEY (VRSN_SEQ_ID),
	CONSTRAINT FK_PLC_ENT_VRSN_TBL FOREIGN KEY (VRSN_SEQ_ID) REFERENCES ENT_VRSN_TBL(VRSN_SEQ_ID)
);

-- PLACE SERVICE TABLE
CREATE TABLE PLC_SVC_TBL (
	SVC_ID UUID NOT NULL,
	ENT_ID INTEGER NOT NULL, -- THE ENTITY TO WHICH THE SERVICE APPLIES
	...
	SVC_CD_ID INTEGER NOT NULL, -- THE CONCEPT WHICH DEFINES THE SERVICE
...
CREATE TABLE ORG_TBL (
	VRSN_SEQ_ID INTEGER NOT NULL, -- THE ENTITY VERSION TO WHICH THIS ORG APPLIES
	IND_CD_ID INTEGER, -- THE INDUSTRY CONCEPT OF THE ORGANIZATION
	CONSTRAINT PK_ORG_TBL PRIMARY KEY (VRSN_SEQ_ID),
	CONSTRAINT FK_ORG_VRSN_SEQ_ID FOREIGN KEY (VRSN_SEQ_ID) REFERENCES ENT_VRSN_TBL(VRSN_SEQ_ID), 
...
CREATE TABLE APP_ENT_TBL (
	VRSN_SEQ_ID INTEGER NOT NULL, -- THE ENTITY TO WHICH THE VERISON APPLIES
	...
	CONSTRAINT PK_APP_ENT_TBL PRIMARY KEY (VRSN_SEQ_ID),
	CONSTRAINT FK_APP_ENT_VRSN_SEQ_ID FOREIGN KEY (VRSN_SEQ_ID) REFERENCES ENT_VRSN_TBL(VRSN_SEQ_ID),
...
CREATE TABLE DEV_ENT_TBL (
	VRSN_SEQ_ID INTEGER NOT NULL, -- THE ENTITY VERSION TO WHICH THE DEVICE INFORMATION APPLIES
	...
	CONSTRAINT PK_DEV_ENT_TBL PRIMARY KEY (VRSN_SEQ_ID),
	CONSTRAINT FK_DEV_ENT_VRSN_SEQ_ID FOREIGN KEY (VRSN_SEQ_ID) REFERENCES ENT_VRSN_TBL(VRSN_SEQ_ID),
...
CREATE TABLE MAT_TBL (
	VRSN_SEQ_ID INTEGER NOT NULL, -- THE ENTITY VERSION TO WHICH TH EMATERIAL DATA APPLIES
	EXP_UTC DATE, -- THE DATE WHEN THE MATERIAL EXPIRES
	FRM_CD_ID INTEGER, -- THE FORM OF THE MATERIAL (CAPSULES, INJECTION, ETC),
	QTY DECIMAL(18,4), -- THE QUANTITY OF THIS OBJECT IN THE PARENT OBJECT
	QTY_CD_ID INTEGER, -- IDENTIFIES THE QUANTITY UNITS OF MATERIAL 
	...
	CONSTRAINT PK_MAT_TBL PRIMARY KEY (VRSN_SEQ_ID),
	CONSTRAINT FK_MAT_VRSN_SEQ_ID FOREIGN KEY (VRSN_SEQ_ID) REFERENCES ENT_VRSN_TBL(VRSN_SEQ_ID),
...
CREATE TABLE MMAT_TBL (
	VRSN_SEQ_ID INTEGER NOT NULL, -- THE MATERIAL VERSION ID
	LOT_NO VARCHAR(32), -- THE LOT NUMBER OF THE MMAT
	CONSTRAINT PK_MMAT_TBL PRIMARY KEY (VRSN_SEQ_ID),
	CONSTRAINT FK_MMAT_VRSN_SEQ_ID FOREIGN KEY (VRSN_SEQ_ID) REFERENCES MAT_TBL(VRSN_SEQ_ID)
);
...
CREATE TABLE PSN_TBL (
	VRSN_SEQ_ID INTEGER NOT NULL, -- THE ENTITY VERSION ID TO WHICH TH EPERSON INFO 
	DOB DATE, -- DATE OF BIRTH OF THE PERSON
	DOB_PREC CHAR(1) CHECK (DOB_PREC IN ('Y','M','D')), -- THE PRECISION OF TH EDATE
	CONSTRAINT PK_PSN_TBL PRIMARY KEY (VRSN_SEQ_ID),
	CONSTRAINT FK_PSN_VRSN_SEQ_ID FOREIGN KEY (VRSN_SEQ_ID) REFERENCES ENT_VRSN_TBL(VRSN_SEQ_ID)
);

-- PERSON LANGUAGE COMMUNICATION TABLE
CREATE TABLE PSN_LNG_TBL (
	LNG_ID UUID NOT NULL,
	ENT_ID INTEGER NOT NULL, -- THE PERSON TO WHICH THE LANGUAGE APPLIES
...
CREATE TABLE PAT_TBL (
	VRSN_SEQ_ID INTEGER NOT NULL, -- THE VERSION ID OF THE ENTITY VERSION TO WHICH THE PATIENT INFO APPLIES
	GNDR_CD_ID INTEGER NOT NULL, -- THE GENDER OF THE PATIENT
	...
	MRTL_STS_CD_ID INTEGER, -- MARITAL STATUS KEY
	EDU_LVL_CD_ID INTEGER, -- EDUCATION LEVEL KEY
	LVN_ARG_CD_ID INTEGER, -- LIVING ARRANGEMENT
	RLGN_CD_ID INTEGER, -- RELIGION CODE
	ETH_GRP_CD_ID INTEGER, -- ETHNIC GROUP KEY
	CONSTRAINT PK_PAT_TBL PRIMARY KEY (VRSN_SEQ_ID),
	CONSTRAINT FK_PAT_VRSN_SEQ_ID FOREIGN KEY (VRSN_SEQ_ID) REFERENCES PSN_TBL(VRSN_SEQ_ID),
...
CREATE TABLE PVDR_TBL (
	VRSN_SEQ_ID INTEGER NOT NULL, -- THE VERSION OF THE PERSON TO WHICH THE PROVIDER INFO APPLIES
	SPEC_CD_ID INTEGER, -- THE PROVIDER'S SPECIALTY
	CONSTRAINT PK_PVDR_TBL PRIMARY KEY (VRSN_SEQ_ID),
	CONSTRAINT FK_PVDR_VRSN_SEQ_ID FOREIGN KEY (VRSN_SEQ_ID) REFERENCES PSN_TBL(VRSN_SEQ_ID),
	CONSTRAINT FK_PVDR_SPEC_CD_ID FOREIGN KEY (SPEC_CD_ID) REFERENCES CD_TBL(CD_ID)
);

-- USER ENTITY TABLE
CREATE TABLE USR_ENT_TBL (
	VRSN_SEQ_ID INTEGER NOT NULL, -- THE ENTITY VERSION OF THE PERSON THE ENTITY APPLIES TO
	SEC_USR_ID UUID NOT NULL, -- THE SECURITY USER IDENTIFIER OF THE USER ENTITY
	CONSTRAINT PK_USR_ENT_TBL PRIMARY KEY (VRSN_SEQ_ID),
	CONSTRAINT FK_USR_VRSN_SEQ_ID FOREIGN KEY (VRSN_SEQ_ID) REFERENCES PSN_TBL(VRSN_SEQ_ID),
...
CREATE TABLE ACT_PROTO_ASSOC_TBL (
	PROTO_ID UUID NOT NULL, 
	ACT_ID INTEGER NOT NULL, -- THE ACT TO WHICH THE PROTOCOL APPLIES
...
CREATE TABLE ACT_NOTE_TBL (
	...
	ACT_ID INTEGER NOT NULL, -- THE ACT TO WHICH THE NOTE APPLIES
	...
	AUTH_ENT_ID INTEGER NOT NULL, -- THE UAHTOR WHO CREATED THE NOTE (SINCE A NOTE CAN BE ATTACHED WITHOUT CREATING A NEW VERSION)
...
CREATE TABLE ENT_NOTE_TBL (
	NOTE_ID UUID NOT NULL,
	ENT_ID INTEGER NOT NULL, -- THE ENTITY TO WHICH THE NOTE APPLIES
...
CREATE TABLE ACT_PTCPT_TBL (
	...
	ENT_ID INTEGER NOT NULL, -- THE ENTITY TO WHICH THE PARTICIPATION APPLIES
	ACT_ID INTEGER NOT NULL, -- THE ACT TO WHICH THE PARTICIPATION APPLIES
	...
	ROL_CD_ID INTEGER NOT NULL, -- THE ROLE THAT THE ENTITY PLAYS IN ACT
	ACT_PTCPT_SEQ_ID INTEGER NOT NULL,
...
BEGIN
	FOR SELECT '1.9.0.0' FROM RDB$DATABASE
		INTO :VER
	DO BEGIN
		SUSPEND;
	END
...
CREATE TABLE ent_rel_vrfy_cdtbl (
	...
	rel_typ_CD_ID INTEGER NOT NULL, -- THE TYPE OF RELATIONSHIP
	src_cls_CD_ID INTEGER NOT NULL, -- THE CLASS CODE OF THE SOURCE ENTITY
	trg_cls_CD_ID INTEGER NOT NULL, -- THE CLASS CODE OF THE TARGET ENTITY
...
CREATE FUNCTION CD_SET_XREF(
	CD_SET_UUID_IN UUID
) RETURNS INTEGER AS 
BEGIN
	RETURN (SELECT SET_ID FROM CD_SET_TBL WHERE UUID = :CD_SET_UUID_IN);
END!! 


CREATE FUNCTION REF_TERM_XREF(
	REF_TERM_UUID_IN UUID
) RETURNS INTEGER AS 
BEGIN
	RETURN (SELECT REF_TERM_ID FROM REF_TERM_TBL WHERE UUID = :REF_TERM_UUID_IN);
END!! 
SET TERM ; !!
