...
namespace MARC.HI.EHRS.SVC.Presentation.Configuration
{
    ...
    /// </summary>
    public class ServiceConfigurationPanel : IConfigurableFeature
    {
        ...
        public void Configure(System.Xml.XmlDocument configurationDom)
        {
            ...
            try
            {
                ServiceTools.ServiceInstaller.Install("SanteDB Host Process", "Host Process for SanteDB", Path.Combine(Path.GetDirectoryName(Assembly.GetEntryAssembly().Location), "SanteDB.exe"), this.AccountName, this.AccountPassword == null ? null : this.AccountPassword.ToString(), this.Mode);
            }
            catch (Exception e)
            {
        ...
        {
            bool isInstalled = ServiceTools.ServiceInstaller.ServiceIsInstalled("SanteDB Host Process");
            if (isInstalled)
            {
                var svcInfo = ServiceTools.ServiceInstaller.GetServiceConfig("SanteDB Host Process");
                if (svcInfo != null)
                {
        ...
        public string Name
        {
            get { return "SanteDB/Windows Service"; }
        }
