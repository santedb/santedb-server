...
#pragma warning disable CS0067
    public class ADODataWarehouse : IAdHocDatawarehouseService
    {
        ...
        /// </summary>
        [PolicyPermission(SecurityAction.Demand, PolicyId = PermissionPolicyIdentifiers.QueryWarehouseData)]
        public IEnumerable<dynamic> StoredQuery(Guid datamartId, string queryId, dynamic queryParameters, int offset, int count, out int totalResults)
        {
            this.ThrowIfDisposed();
            ...
            using (var context = this.m_configuration.Provider.GetReadonlyConnection())
            {
                try
                {
                    ...
                    return this.QueryInternal(context, String.Format("sqp_{0}_{1}", mart.Schema.Name, queryId), queryDefn.Properties, parms, offset, count, out totalResults);
                }
                catch (Exception e)
                {
